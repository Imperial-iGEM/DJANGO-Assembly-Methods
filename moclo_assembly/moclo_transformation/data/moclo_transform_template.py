...
'''
    Up to 88 rxns per single run and 24 rxns for triplicate of each combination
    of DNA assembly
...
# num_plates = math.ceil(num_rxns/24) # Amount of agar plates need for plating
# the transformed cells

'''
    For this protocol, the biorad_96_wellplate_200ul_pcr, is placed on the top
    of the TempDeck alone without the Opentrons 96 well aluminum block.
'''

# Load in Bio-Rad 96 Well Plate on temp deck for moclos,
# transformation, and outgrowth.
temp_deck = modules.load('tempdeck', '10')
reaction_plate = labware.load('biorad_96_wellplate_200ul_pcr', '10',
                              share=True)
temp_deck.set_temperature(10)

# Load in 1 10ul tiprack and 2 300ul tipracks
tr_10 = [labware.load('opentrons_96_tiprack_10ul', '3'),
         labware.load('opentrons_96_tiprack_10ul', '6')]
tr_300 = [labware.load('opentrons_96_tiprack_300ul', '8'),
...
'''
    This deck slot location is dedicated for the reaction plate after MoClo
    protocol is completed, so at the beginning of the protocol there isn't
    an actual plate existing in this slot location.
'''
...
for plate_name in dna_plate_map_dict.keys():
    dna_plate_dict[plate_name] = labware.load('biorad_96_wellplate_200ul_pcr',
                                              '1', 'Input DNA Plate')

# Load in 1 agar plate, same antibiotic for all plasmids is assumed (e-gelgol)
# Be careful labware e-gelgol is an old container type, and could eventually
# be removed by Opentrons!
agar_plate = labware.load('e-gelgol', '2', 'Agar Plate')

# available_deck_slots = ['11', '8']

# ______________Start the MoClo protocol______________

'''
    For this protocol, the biorad_96_wellplate_200ul_pcr, is placed on the top
    of the TempDeck alone without the metal part.
'''
...
for i in range(2):
    ...
    p10_single.transfer((150), water.bottom(),
                        reaction_plate.wells(95-i).bottom(0.5),
                        new_tip='never')  # Water
    p10_single.blow_out()
    p10_single.drop_tip()
    p10_single.pick_up_tip()
    p10_single.transfer(24, buffer.bottom(),
                        reaction_plate.wells(95-i).bottom(0.5),
                        new_tip='never')  # Buffer (1uL/rxn)
    p10_single.mix(2, 10, reaction_plate.wells(95-i).bottom(0.5))
    p10_single.blow_out()
    p10_single.drop_tip()
    p10_single.pick_up_tip()
    p10_single.transfer(6, ligase.bottom(),
                        reaction_plate.wells(95-i).bottom(0.5),
                        new_tip='never')  # Ligase (0.25uL/rxn)
    p10_single.mix(2, 10, reaction_plate.wells(95-i).bottom(0.5))
    p10_single.blow_out()
    p10_single.drop_tip()
    p10_single.pick_up_tip()
    p10_single.transfer(12, restriction_enzyme.bottom(),
                        reaction_plate.wells(95-i).bottom(0.5),
                        new_tip='never')  # Restriction Enzyme  (0.5uL/rxn)
    p10_single.mix(2, 10, reaction_plate.wells(95-i).bottom(0.5))
    p10_single.blow_out()
    p10_single.drop_tip()
...
for i in range(2):
    p10_single.pick_up_tip()
    p10_single.transfer((78), water.bottom(),
                        reaction_plate.wells(93-i).bottom(0.5),
                        new_tip='never')  # Water
    p10_single.blow_out()
    p10_single.drop_tip()
    p10_single.pick_up_tip()
...
for i in range(2):
    p10_single.pick_up_tip()
    p10_single.transfer((6), water.bottom(),
                        reaction_plate.wells(91-i).bottom(0.5),
                        new_tip='never')  # Water
    p10_single.blow_out()
    p10_single.drop_tip()
    p10_single.pick_up_tip()
...
p10_single.pick_up_tip()
for i in range(12):
    p10_single.transfer(16, reaction_plate.wells(95).bottom(),
                        reaction_plate.wells(i).bottom(0.3), new_tip='never')
    p10_single.blow_out()
p10_single.drop_tip()

p10_single.pick_up_tip()
for i in range(12):
    p10_single.transfer(16, reaction_plate.wells(94).bottom(),
                        reaction_plate.wells(12+i).bottom(0.3),
                        new_tip='never')
    p10_single.blow_out()
p10_single.drop_tip()
...
p10_single.pick_up_tip()
for i in range(12):
    p10_single.transfer(10, reaction_plate.wells(93).bottom(),
                        reaction_plate.wells(24+i).bottom(0.3),
                        new_tip='never')
    p10_single.blow_out()
p10_single.drop_tip()
...
p10_single.pick_up_tip()
for i in range(12):
    p10_single.transfer(10, reaction_plate.wells(92).bottom(),
                        reaction_plate.wells(36+i).bottom(0.3),
                        new_tip='never')
    p10_single.blow_out()
p10_single.drop_tip()
...
p10_single.pick_up_tip()
for i in range(12):
    p10_single.transfer(4, reaction_plate.wells(91).bottom(),
                        reaction_plate.wells(48+i).bottom(0.3),
                        new_tip='never')
    p10_single.blow_out()
p10_single.drop_tip()
...
p10_single.pick_up_tip()
for i in range(12):
    p10_single.transfer(4, reaction_plate.wells(90).bottom(),
                        reaction_plate.wells(60+i).bottom(0.3),
                        new_tip='never')
    p10_single.blow_out()
p10_single.drop_tip()
...
'''
    ...
    for j in range(8):
    p10_single.pick_up_tip()
    p10_single.transfer(4*(2**i), reaction_plate.wells(89+(3*i)).bottom(0.3),
                        reaction_plate.wells(3*j+count).bottom(0.3),
                        new_tip='never')
    p10_single.blow_out()
    p10_single.drop_tip()
    count += 1
    ...
    for j in range(8):
    p10_single.pick_up_tip()
    p10_single.transfer(2.5*(2**i), reaction_plate.wells(88+(3*i)).bottom(0.3),
                        reaction_plate.wells(27+3*j+count).bottom(0.3),
                        new_tip='never')
    p10_single.blow_out()
    p10_single.drop_tip()
    count += 1
    ...
    for j in range(8):
    p10_single.pick_up_tip()
    p10_single.transfer(1*(2**i), reaction_plate.wells(87+(3*i)).bottom(0.3),
                        reaction_plate.wells(54+3*j+count).bottom(0.3),
                        new_tip='never')
    p10_single.blow_out()
    p10_single.drop_tip()
    count += 1
...
# This function checks the existance of DNA parts and returns for well
...
# This function checks if the DNA parts exist in the DNA plates and returns for
# well locaion of output DNA combinations
...
# This section of the code combines and mix the DNA parts according to the
# combination list
...
'''
    ...
    Discard the empty PCR tubes previously containing buffer, ligase, and
    restriction enzyme.
    Remove the Input_DNA_Plate from the Deck Space. Remaining DNA may be saved
    by sealing the Input_DNA_Plate with adhesive film and storing at -20°C
'''
...
'''
    ...
    p10_single.transfer(water_to_add, water.bottom(), well.bottom(0.5),
                        new_tip='never')
...
# _____________________Start the Transformation protocol__________________
'''
    The robotic liquid handler would automatically pause when the modular
    cloning protocol is completed, indicated by the blue light (cooling down)
    of the Temperature Module.
    Remove the reaction_plate from the top of the Temperature Module and place
    it on Slot 7 which is assigned to the post_moclo_reaction_plate at the
    beginning of the protocol, and remove the adhesive film.
    Before beginning the Cell Transformation protocol, load 10 μL/well of
    chemically competent cells into a new Bio-Rad 96 Well Plate and place it
    on the top of the Temperature Module.
    Be sure to un-pause the robot after completing all the steps listed above!
'''

# for i in range(0, num_cols):
# Using letters for rows of custom container to maintain backwards
# compatibility.
...
for i in range(0, num_rxns):
    p10_single.transfer(2, post_moclo_reaction_plate.wells(i).bottom(0.5),
                        reaction_plate.wells(i).bottom(0.5),
                        new_tip='never')
...
num_cols = len(reaction_plate.cols())
for i in range(0, num_cols):
    p300_multi.transfer(150, soc.bottom(), reaction_plate.cols(i).bottom(1),
                        new_tip='never')
...
temp_deck.deactivate()
'''
    Remove the adhesive film from the Reaction Plate before perceeding to Cell
    Plating.
...
'''
    All recovered transformation reactions are diluted to 10% of its original
    concentration before plating
'''
# Dilution
p300_multi.pick_up_tip()
for i in range(0, num_cols):
    p300_multi.transfer(157, reaction_plate.cols(i).bottom(0.5),
                        liquid_waste.bottom(), new_tip='never')
    p300_multi.blow_out()
    p300_multi.mix(2, 300, wash_0.bottom())
    p300_multi.blow_out()
    p300_multi.mix(2, 300, wash_1.bottom())
    p300_multi.blow_out()
    # Two washing steps are added to allow recycling of the tips
    p300_multi.transfer(45, soc.bottom(), reaction_plate.cols(i).bottom(0.5),
                        new_tip='never')
...
# Plating
'''
    Un-comment line 328, and line 333 through line 339 if you're plating for
    triplicate of each reactions.
'''
# count2 = 0
for i in range(0, num_cols):
    p300_multi.pick_up_tip()
    p300_multi.transfer(10, reaction_plate.cols(i).bottom(0.5),
                        agar_plate.cols(i).bottom(0.3), new_tip='never')
    # p300_multi.transfer(1, reaction_plate.cols(i).bottom(0.5),
    #                     agar_plate.cols(i).bottom(-1), new_tip='never')
    # p300_multi.transfer(9, reaction_plate.cols(i).bottom(0.5),
    #                     agar_plate.cols(i+count2).bottom(0.8),
    #                     new_tip='never')
    # p300_multi.transfer(1, reaction_plate.cols(i).bottom(0.5),
    #                     agar_plate.cols(i+count2).bottom(-1),
    #                     new_tip='never')
    # p300_multi.transfer(9, reaction_plate.cols(i).bottom(0.5),
    #                     agar_plate.cols(i+count2+1).bottom(0.8),
    #                     new_tip='never')
    # p300_multi.transfer(1, reaction_plate.cols(i).bottom(0.5),
    #                     agar_plate.cols(i+count2+1).bottom(-1),
    #                     new_tip='never')
    # p300_multi.transfer(9, reaction_plate.cols(i).bottom(0.5),
    #                     agar_plate.cols(i+count2+2).bottom(0.8),
    #                     new_tip='never')
    # p300_multi.transfer(1, reaction_plate.cols(i).bottom(0.5),
    #                     agar_plate.cols(i+count2+2).bottom(-1),
    #                     new_tip='never')
    # count2 += 2
    p300_multi.drop_tip()
...
'''
    ...
    def spread_culture(source, dest, soc, dilute_after=True):
        ...
        if dilute_after:
            p300_multi.transfer(120, source.bottom(0.5), liquid_waste.bottom(),
                                new_tip='never')
            p300_multi.mix(2, 300, wash_0.bottom())
            p300_multi.blow_out()
            p300_multi.mix(2, 300, wash_1.bottom())
        ...
        for i in range(0, num_cols):
            ...
            spread_culture(source, agar_plate.wells(agar_well_num + 24), soc,
                           dilute_after=False)
            p300_multi.drop_tip()
'''
