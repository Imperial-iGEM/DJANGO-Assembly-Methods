...
def moclo_function(output_folder,single_or_triplicate,dna_plate_map,combinations_file):
	...
	config = {'output_folder_path': output_folder,
          	  'protocol_template_path': 'moclo_assembly/moclo_transformation/data/moclo_transform_template.py'}
...
def generate_plate_maps(myfilename):
	#print('myfilename: {}'.format(myfilename))
	filename = os.path.abspath(myfilename[1:])
...
def generate_combinations(mycombinations_filename):
	combinations_to_make = []
	combinations_filename = os.path.abspath(mycombinations_filename[1:])
...
def generate_and_save_output_plate_maps(combinations_to_make, combinations_limit, output_folder_path):
	...
	#print('output folder path: {}'.format(output_folder_path))
	myoutput_filename = os.path.join(output_folder_path, "Agar_plate.csv")
	#print('myoutput filename: {}'.format(myoutput_filename))
	output_filename = os.path.abspath(myoutput_filename)
	#print('output filename: {}'.format(output_filename))
	os.makedirs(os.path.abspath(output_folder_path))
	with open(output_filename, 'w+', newline='') as f:
		writer = csv.writer(f)          
		for row in output_plate_map:
			writer.writerow(row)

def create_protocol(dna_plate_map_dict, combinations_to_make, myprotocol_template_path, output_folder_path):
	
	# Get the contents of colony_pick_template.py, which contains the body of the protocol.
	protocol_template_path = os.path.abspath(myprotocol_template_path)
