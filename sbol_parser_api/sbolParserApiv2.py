...
class ParserSBOL:
    ...
    def generateCsv_for_DNABot(
        ...
        for plate in constructPlates:
            ...
            constructDf = self.getConstructCsvFromPlateoPlate(plate, uniqueId)
            # Write parts/linkers csv
            partPlate, countParts = self.getPartLinkerCsvFromPlateoPlate(
                plate, linkerFile, uniqueId, assembly)
            partPlates.extend(partPlate)

        return constructPlates, partPlate, countParts
    ...
    def generateCsv_for_BioBricks(
            ...
            listOfNonCombUris: List[str],
            listOfCombUris: List[str],
            partFile: sbol2.document.Document
    ):
        allConstructs = []
        ...
        for plate in constructPlates:
            ...
            constructDf = self.getConstructCsvFromPlateoPlate(plate, uniqueId,
                                                              assembly)
            # Write parts/linkers csv
            partPlate, partDf = self.getPartLinkerCsvFromPlateoPlate(
                    plate, partFile, uniqueId, assembly)

        return constructPlates[0], partPlate, constructDf, partDf
    ...
    def getListOfParts(
        ...
        for construct in allConstructs:
            for index, component in enumerate(construct.components):
                part = self.doc.getComponentDefinition(component.definition)
                listOfParts.append(part)
                if assembly == 'BASIC':
                    if self.isLinker(part, linkerFile):
                        for linker in self.getLinkerSP(part):
                            if linker in countParts.keys():
                                countParts[linker] += 1
                            else:
                                countParts[linker] = 1
                    else:
                        if part in countParts.keys():
                            countParts[part] += 1
                        else:
                            countParts[part] = 1
                else:
                ...
                if assembly == 'BioBricks':
                    ...
                    countPlasmid = {}
                    if index == 0:
                        if part in countUpstream.keys():
                            countUpstream[part] += 1
                        else:
                            countUpstream[part] = 1
                    elif index == 1:
                        if part in countDownstream.keys():
                            countDownstream[part] += 1
                        else:
                            countDownstream[part] = 1
                    elif index == 2:
                        if part in countPlasmid.keys():
                            countPlasmid[part] += 1
                        else:
                            countPlasmid[part] = 1
        ...
        if assembly == 'BioBricks':
            return listOfParts, countParts, countUpstream, countDownstream, \
                countPlasmid
        else:
            return listOfParts, countParts
    ...
    def getConstructCsvHeader(
        ...
        if assembly == 'BASIC':
            ...
            header = ["Well", "Upstream", "Downstream", "Plasmid"]
        return header
    ...
    def getConstructDfFromPlateoPlate(
        ...
        if assembly == 'BASIC':
            minNumberOfBasicParts = self.getMinNumberOfBasicParts(
                allComponents)
            header = self.getConstructCsvHeader(assembly,
                                                minNumberOfBasicParts)
        else:
            header = self.getConstructCsvHeader(assembly)
        dictWellComponent = self.getWellComponentDictFromPlateoPlate(
            constructPlate)
        listWellComponent = self.getListFromWellComponentDict(
            dictWellComponent)
    ...
    def getDisplayIDPartDict(self, listOfParts):
        displayIDPartDict = {}
        for part in listOfParts:
            displayIDPartDict[part.displayId] = part
        return displayIDPartDict
    ...
    def getPartLinkerCsvFromPlateoPlate(
        ...
        if assembly == 'BioBricks':
            listOfParts, countParts, countUpstream, countDownstream, \
                 countPlasmid = self.getListOfParts(linkerFile, allConstructs,
                                                    assembly)
        else:
            listOfParts, countParts = self.getListOfParts(linkerFile,
                                                          allConstructs,
                                                          assembly)
        if assembly == 'BASIC':
            # Change linker to linker-s and linker-p
            listOfParts, countParts = self.convertLinkerToSuffixPrefix(
                listOfParts, linkerFile)
        ...
        # fill out DF if assembly type is BioBricks
        if assembly == 'BioBricks':
            displayIDPartDict = self.getDisplayIDPartDict(listOfParts)
            partLinkerDf['counts'] = pd.Series([] * len(partLinkerDf.index))
            partLinkerDf['upstreamCounts'] = pd.Series(
                [] * len(partLinkerDf.index))
            partLinkerDf['downstreamCounts'] = pd.Series(
                [] * len(partLinkerDf.index))
            partLinkerDf['plasmidCounts'] = pd.Series(
                [] * len(partLinkerDf.index))
            partCountsList = []
            upstreamCountsList = []
            downstreamCountsList = []
            plasmidCountsList = []
            for index, row in partLinkerDf.iterrows():
                part = displayIDPartDict[row["Part/linker"]]
                partCountsList.append(countParts[part])
                upstreamCountsList.append(countUpstream[part])
                downstreamCountsList.append(countDownstream[part])
                plasmidCountsList.append(countPlasmid[part])
            partLinkerDf['counts'] = partCountsList
            partLinkerDf['upstreamCounts'] = upstreamCountsList
            partLinkerDf['downstreamCounts'] = downstreamCountsList
            partLinkerDf['plasmidCounts'] = plasmidCountsList

        if assembly == 'BioBricks':
            # return DF with additional info
            return partPlates, partLinkerDf

        else:
            return partPlates, countParts
